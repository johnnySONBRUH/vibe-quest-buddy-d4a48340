...
const categoryColors: Record<string, string> = {
  study: 'bg-blue-500/15 text-blue-400 border-blue-500/30',
  exercise: 'bg-green-500/15 text-green-400 border-green-500/30',
...
const MissionHistory = ({ onBack }: { onBack: () => void }) => {
  ...
  const filtered = useMemo(() => {
    ...
    return missions.filter(m => m.missions.category === filter);
  }, [missions, filter]);
  ...
  const categoryCounts = useMemo(() => {
    ...
    missions.forEach(m => {
      counts[m.missions.category] = (counts[m.missions.category] || 0) + 1;
    });
    return counts;
  }, [missions]);
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-2xl mx-auto p-4 sm:p-6 pb-24 space-y-6">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-foreground">Mission History</h1>
            <p className="text-sm text-muted-foreground">{missions.length} missions completed</p>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }} className="grid grid-cols-3 gap-3">
          ...
          <div className="glass-card rounded-xl p-4 text-center">
            ...
            <p className="text-lg font-bold text-foreground capitalize mt-1">{topCategory?.[0] || '—'}</p>
        ...
        {grouped.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="font-medium">No completed missions yet</p>
            <p className="text-sm">Complete your daily missions to see them here!</p>
          </div>
        ) : (
