...
const xpConfig: ChartConfig = {
  xp: { label: 'XP Earned', color: 'hsl(var(--primary))' },
};

const streakConfig: ChartConfig = {
  streak: { label: 'Streak', color: 'hsl(var(--streak))' },
};
    ...
    const fetchWeekData = async () => {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        days.push({
          ...
          xp: missions.reduce((sum, m) => sum + (m.xp_earned || 0), 0),
        });
      }

      setWeekData(days);

      // Build streak history from profile completion dates
      // We approximate streak by checking consecutive completion days
      ...
      const sData = days.map((d, idx) => {
        // Estimate: if all missions done that day, streak was active
        const allDone = d.total > 0 && d.completed === d.total;
        // Build approximate streak values going backward from current
