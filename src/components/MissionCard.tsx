...
const MissionCard = ({ dailyMission, onComplete, index }: MissionCardProps) => {
  ...
  return (
    <motion.div
      ...
      className={`rounded-xl border p-4 ${categoryColors[cat]} transition-all ${
        completed ? 'opacity-70' : 'hover:shadow-md cursor-pointer'
      }`}
      onClick={() => !completed && onComplete(dailyMission.id)}
    >
      <div className="flex items-center gap-4">
        <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${
          completed ? 'bg-success/20' : `${categoryColors[cat]}`
        }`}>
          {completed ? (
        ...
        <div className="flex-1 min-w-0">
          <h3 className={`font-semibold ${completed ? 'line-through text-muted-foreground' : 'text-foreground'}`}>
            {mission.title}
          </h3>
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            <span className="text-xs text-muted-foreground">
              Lvl {difficulty_level}
            </span>
          </div>
        </div>

        <div className="text-right">
          <span className={`text-lg font-bold ${completed ? 'text-success' : 'text-accent-foreground'}`}>
            +{xpValue}
          </span>
